 ...
 */
function normalizePhone(rawPhone: string): { e164: string; msg91Recipient: string; phone10: string } {
  ...
  throw new HttpsError('invalid-argument', 'Please provide a valid phone number (10 to 15 digits).');
}
...
export const sendWhatsAppOtp = onCall(
  ...
  async (request) => {
    ...
    if (!authKey) {
      ...
      throw new HttpsError('internal', 'WhatsApp service is temporarily unavailable.');
    }
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to deliver OTP to WhatsApp. Please try again.');
    }
...
export const verifyWhatsAppOtp = onCall(
  ...
  async (request) => {
    ...
    if (!sessionSnap.exists) {
      throw new HttpsError('not-found', 'OTP session not found or expired.');
    }
    ...
    if (phone) {
      ...
      if (session.phone10 && session.phone10 !== phone10) {
        throw new HttpsError('invalid-argument', 'Phone number mismatch.');
      }
    }
    ...
    if (expectedHash !== session.hashedOtp) {
      ...
      throw new HttpsError('invalid-argument', 'Invalid OTP. Please check and try again.');
    }
